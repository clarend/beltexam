...
class TripManager(models.Manager):
    def add_trip(self, request):
        ...
        Trip.objects.create(destination=request.POST['destination'], desc=request.POST['desc'], start_date=request.POST['start_date'], end_date=request.POST['end_date'], planner=user)
    ...
    def trip_validations(self, request):
        ...
        if start_date:
            ...
            if start_date < date.today():
                errors.append(' Incorrect Start date: must be today or in the future.')
        else:
        ...
        if end_date:
            ...
            if end_date < date.today():
                errors.append('Incorrect End date: must be today or in the future.')
        else:
            errors.append('Please add an end date.')

        if start_date and end_date:
            if start_date > end_date:
                errors.append('Incorrect End date: must be after or the same as start date.')
